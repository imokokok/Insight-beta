...
export class ComplianceReportGenerator {
  private readonly FRAMEWORKS: Record<ComplianceReportType, FrameworkConfig> = {
    soc2_type2: {
      ...
      sections: [{ id: 'cc', name: 'Common Criteria', weight: 100, requirements: 116 }],
    },
    iso27001: {
      name: 'ISO 27001',
  ...
  generateReport(
    ...
    const status =
      overallScore >= 95 ? 'compliant' : overallScore >= 75 ? 'needs_review' : 'non_compliant';
  ...
  private generateRequirements(sectionId: string, count: number): ComplianceRequirement[] {
    ...
    for (let i = 0; i < Math.min(count, templates.length); i++) {
      ...
      const randomStatus = statuses[Math.floor(Math.random() * statuses.length)] ?? 'partial';
