 ...
 * 将百分比字符串解析为小数
 *
 * @param percentStr - 百分比字符串，如 '12.34%' 或 '12.34'
 * @returns 小数（0-1），解析失败返回 null
 *
 * @example
 * ```typescript
 * parsePercent('12.34%');  // Returns: 0.1234
 * parsePercent('12.34');   // Returns: 0.1234
 * parsePercent('invalid'); // Returns: null
 * ```
 */
export function parsePercent(percentStr: string): number | null {
 ...
 *
 * @param percentStr - 百分比字符串，如 '12.34%' 或 '12.34'
 * @returns 百分比数值（0-100），解析失败返回 null
 *
 * @example
 * ```typescript
 * parsePercentValue('12.34%');  // Returns: 12.34
 * parsePercentValue('12.34');   // Returns: 12.34
 * ```
 */
export function parsePercentValue(percentStr: string): number | null {
  if (!percentStr || typeof percentStr !== 'string') {
    return null;
  }
 ...
 * @example
 * ```typescript
 * calculateAndFormatPercent(25, 100);        // Returns: '25.00%'
 * calculateAndFormatPercent(1, 3, 1);        // Returns: '33.3%'
 * calculateAndFormatPercent(0, 0);           // Returns: '—'
 * ```
 */
export function calculateAndFormatPercent(
  part: number,
  total: number,
  decimals: number = 2,
  options: FormatPercentOptions = {}
  ...
  return formatPercentValue(percent, decimals, options);
}
 ...
 * @param previous - 之前的值
 * @param decimals - 小数位数，默认 2
 * @returns 格式化后的变化率字符串，如 '+12.34%' 或 '-5.67%'
 *
 * @example
 * ```typescript
 * calculateChangePercent(112.34, 100);   // Returns: '+12.34%'
 * calculateChangePercent(94.33, 100);    // Returns: '-5.67%'
 * calculateChangePercent(100, 0);        // Returns: '—'
 * ```
 */
export function calculateChangePercent(
  ...
  if (previous === 0 || !Number.isFinite(current) || !Number.isFinite(previous)) {
    return '—';
  }

  const change = (current - previous) / previous;
  return formatChangePercent(change, decimals, true);
}
