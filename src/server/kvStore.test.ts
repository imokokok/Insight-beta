...
describe('kvStore', () => {
  ...
  describe('readJsonFile', () => {
    it('returns stored value if key exists', async () => {
      ...
      expect(query).toHaveBeenCalledWith(expect.stringContaining('SELECT value FROM kv_store'), [
        'test-key',
      ]);
    });
  ...
  describe('writeJsonFile', () => {
    it('writes value to db', async () => {
      ...
      expect(query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO kv_store'), [
        'test-key',
  ...
  describe('deleteJsonKey', () => {
    it('deletes key from db', async () => {
      ...
      expect(query).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM kv_store'), [
        'test-key',
      ]);
    });
  });
  ...
  describe('listJsonKeys', () => {
    it('lists keys with pagination', async () => {
      ...
      expect(query).toHaveBeenCalledWith(expect.stringContaining('LIMIT $1 OFFSET $2'), [10, 5]);
    });
    ...
    it('filters by prefix', async () => {
      ...
      expect(query).toHaveBeenCalledWith(expect.stringContaining('key LIKE $1'), ['test%']);
    });
  });
});
