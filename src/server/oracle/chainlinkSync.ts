...
export class ChainlinkSyncManager {
  ...
  async startSync(instanceId: string): Promise<void> {
    if (this.isRunning.get(instanceId)) {
      logger.warn(`Chainlink sync already running for instance ${instanceId}`);
      return;
    }
    ...
    logger.info(`Starting Chainlink sync for instance ${instanceId}`, {
  ...
  stopSync(instanceId: string): void {
    ...
    logger.info(`Stopped Chainlink sync for instance ${instanceId}`);
  }
  ...
  private async syncInstance(instanceId: string): Promise<void> {
    ...
    try {
      ...
      // 获取所有可用价格喂价
      const symbols = getAvailableFeedsForChain(instance.chain);

      if (symbols.length === 0) {
        logger.warn(`No available feeds for chain ${instance.chain}`);
        return;
      }

      logger.debug(`Syncing ${symbols.length} feeds for instance ${instanceId}`);
      ...
      logger.info(`Chainlink sync completed for instance ${instanceId}`, {
      ...
      logger.error(`Chainlink sync failed for instance ${instanceId}`, {
        error: errorMessage,
      });
  ...
  private async getInstanceConfig(instanceId: string): Promise<{
    ...
    const result = await query(
      `SELECT id, chain, enabled, config, protocol_config 
       ...
       WHERE id = $1 AND protocol = 'chainlink'`,
      [instanceId],
    );
    ...
    return {
      ...
      enabled: row.enabled,
      config: {
        rpcUrl: row.config?.rpcUrl || '',
        protocolConfig: row.protocol_config || {},
        syncIntervalMs: row.config?.syncIntervalMs,
      },
    };
  }
  ...
  private async updateSyncState(
    ...
    await query(
      ...
      ) VALUES ($1, 'chainlink', $2, 0, 'healthy', 0)
  ...
  async cleanupOldData(): Promise<void> {
    ...
    logger.info(`Cleaning up Chainlink data older than ${SYNC_CONFIG.dataRetentionDays} days`);
