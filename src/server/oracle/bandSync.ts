...
import { createBandClient, getAvailableBandSymbols } from '@/lib/blockchain/bandProtocol';
import type {
  SupportedChain,
  UnifiedPriceFeed,
  UnifiedSyncState,
...
export class BandSyncManager {
  ...
  async startSync(instanceId: string): Promise<void> {
    if (this.isRunning.get(instanceId)) {
      logger.warn(`Band sync already running for instance ${instanceId}`);
      return;
    }
    ...
    logger.info(`Starting Band sync for instance ${instanceId}`, {
  ...
  stopSync(instanceId: string): void {
    ...
    logger.info(`Stopped Band sync for instance ${instanceId}`);
  }
  ...
  private async syncInstance(instanceId: string): Promise<void> {
    ...
    try {
      ...
      // 获取所有可用价格喂价
      const symbols = getAvailableBandSymbols();

      if (symbols.length === 0) {
        logger.warn(`No available feeds for Band on ${instance.chain}`);
        return;
      }

      logger.debug(`Syncing ${symbols.length} feeds for Band instance ${instanceId}`);

      // 批量获取价格（Band 支持批量查询）
      const priceFeeds = await client.getBulkPrices(symbols);
      ...
      logger.info(`Band sync completed for instance ${instanceId}`, {
      ...
      logger.error(`Band sync failed for instance ${instanceId}`, {
        error: errorMessage,
      });
  ...
  async cleanupOldData(): Promise<void> {
    ...
    logger.info(`Cleaning up Band data older than ${SYNC_CONFIG.dataRetentionDays} days`);

    // 清理旧的价格喂价数据
    const feedsResult = await query(
      `DELETE FROM unified_price_feeds
       WHERE protocol = 'band' AND timestamp < $1
       RETURNING id`,
    ...
    logger.info(`Band cleanup completed`, {
