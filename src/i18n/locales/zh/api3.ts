export const api3 = {
  pageDescription: '第一方预言机 - Airnode 技术与签名数据验证',
  tabs: {
    overview: '概览',
    airnodes: 'Airnodes',
    data: '数据服务',
    analysis: '分析工具',
    tools: '工具',
  },
  overview: {
    title: 'API3 协议概览',
    description: '第一方预言机网络状态摘要',
    content:
      'API3 是第一方预言机解决方案，通过 Airnode 技术实现去中心化数据馈送。提供可验证的签名数据和跨链数据服务。',
  },
  features: {
    title: '核心特性',
    airnode: {
      label: 'Airnode 网络',
      value: '去中心化第一方预言机节点',
    },
    signature: {
      label: '签名验证',
      value: '验证数据真实性',
    },
    priceMonitor: {
      label: '价格监控',
      value: '实时价格更新追踪',
    },
  },
  supportedChains: {
    title: '支持的链',
    description: 'API3 支持的区块链网络',
  },
  dapi: {
    listTitle: 'DAPI 列表',
    name: '名称',
    chain: '链',
    price: '价格',
    status: '状态',
    lastUpdate: '最后更新',
    dataFeedId: '数据源 ID',
    sourceType: '数据源类型',
    provider: '提供商',
    refresh: {
      ariaLabel: '刷新数据',
    },
    noData: '暂无 Airnode 数据',
    gasCost: 'Gas 成本',
  },
  provider: {
    title: '数据提供商',
    name: '提供商名称',
    description: '描述',
    website: '网站',
    sourceType: '数据源类型',
    beacon: 'Beacon',
    beaconSet: 'Beacon Set',
  },
  beaconSet: {
    composition: 'BeaconSet 组成',
    beacons: '个 Beacon',
  },
  airnode: {
    title: 'Airnode',
    address: 'Airnode 地址',
    chain: '链',
    responseTime: '响应时间',
    uptime: '运行时间',
    lastSeen: '最后在线',
    endpointId: '端点 ID',
    sponsor: '赞助商地址',
    statusOverview: 'Airnode 状态概览',
    totalAirnodes: '总 Airnodes',
    onlineAirnodes: '在线 Airnodes',
    offlineAirnodes: '离线 Airnodes',
    dapisCount: 'dAPIs 数量',
    lastHeartbeat: '最后心跳',
    responseTimeHistory: '响应时间历史',
    noHistoryData: '暂无历史数据',
    contractAddress: '合约地址',
    ipfsEndpoint: 'IPFS 端点',
    relatedDapis: '关联的 dAPI 列表',
    tradingPair: '交易对',
    currentValue: '当前值',
    updateRecords: '更新记录',
    old_value: '旧值',
    new_value: '新值',
    txHash: '交易哈希',
    online: '在线',
    offline: '离线',
    active: '活跃',
    inactive: '非活跃',
    unknown: '未知',
    viewInExplorer: '在区块浏览器中查看',
    statusInfo: '状态信息',
    airnodeInfo: 'Airnode 信息',
    xpub: 'xpub',
    uptimeRate: '在线率',
    gas: 'Gas',
    dapi: 'dAPI',
  },
  dapiStatus: {
    title: 'dAPIs 状态概览',
    totalDapis: '总 dAPIs',
    activeDapis: '活跃 dAPIs',
    inactiveDapis: '非活跃 dAPIs',
  },
  priceUpdate: {
    totalUpdates: '价格更新事件',
    avgDelay: '平均更新延迟',
    uniqueDapis: '监控 DAPI 数',
    uniqueChains: '监控链数',
    frequencyByDapi: '各 DAPI 更新频率',
    frequencyDesc: '各 DAPI 每分钟价格更新次数',
    delayTrend: '更新延迟趋势',
    delayTrendDesc: '24小时平均更新延迟变化',
    eventList: '价格更新事件列表',
    eventListDesc: '最近的价格更新事件记录',
    dapiName: 'DAPI 名称',
    chain: '链',
    price: '价格',
    delay: '延迟',
    status: '状态',
    timestamp: '时间',
    totalEvents: '总更新事件',
    involvedDapis: '涉及 dAPIs',
    involvedChains: '涉及链',
  },
  signature: {
    title: '签名验证',
    description: '验证 API3 签名数据的真实性',
    signatureLabel: '签名',
    signatureHint: '输入要验证的签名（十六进制格式）',
    dataFeedIdLabel: '数据源 ID',
    timestampLabel: '时间戳',
    airnodeLabel: 'Airnode 地址',
    verifyButton: '验证签名',
    valid: '签名有效',
    invalid: '签名无效',
    signer: '签名者',
    dataFeedId: '数据源 ID',
    timestamp: '时间戳',
    note: '注意',
    noteDescription: '此工具用于验证来自 API3 Airnode 签名数据的真实性。',
    requiredFields: '请填写所有必填字段',
    invalidSignature: '签名格式无效',
  },
  price: {
    description: '来自 API3 DAPI 的实时价格数据',
    currentPrice: '当前价格',
    emaPrice: 'EMA 价格',
    spotPrice: '现货价格',
    custom: '自定义',
    apply: '应用',
  },
  frequency: {
    title: '更新频率分析',
    description: '数据更新频率和异常检测分析',
    avgInterval: '平均更新间隔',
    minInterval: '最小间隔',
    maxInterval: '最大间隔',
    updateCount: '更新次数',
    anomaly: '异常',
    timeRange: '时间范围',
    interval: '更新间隔',
    avg: '平均',
    normalUpdate: '正常更新',
    anomalyUpdate: '异常更新',
    avgLine: '平均线',
  },
  performance: {
    excellent: '优秀',
    good: '良好',
    slow: '较慢',
  },
  alertConfig: {
    title: '告警配置',
    description: '配置 API3 dAPI 监控告警规则',
    addRule: '添加告警规则',
    editRule: '编辑告警规则',
    newRule: '新建告警规则',
    alertType: '告警类型',
    selectAlertType: '选择告警类型',
    alertName: '告警名称',
    enterAlertName: '输入告警名称',
    targetDapi: '目标 dAPI',
    selectDapi: '选择 dAPI',
    targetAirnode: '目标 Airnode',
    enterAirnodeAddress: '输入 Airnode 地址',
    blockchainNetwork: '区块链网络',
    selectNetwork: '选择网络',
    threshold: '阈值',
    enterThreshold: '输入阈值',
    enableAlert: '启用告警',
    alertRulesList: '告警规则列表',
    configuredAlertRules: '已配置的告警规则 ({count})',
    noAlertRules: '暂无告警规则',
    clickToAddAlert: '点击上方按钮添加第一个告警规则',
    seconds: '秒',
    enabled: '启用',
    disabled: '禁用',
    priceDeviation: {
      label: '价格偏差告警',
      description: '当价格偏差超过设定阈值时触发告警',
      thresholdLabel: '偏差阈值 (%)',
    },
    updateFrequency: {
      label: '更新频率异常',
      description: '当更新间隔超过设定时间时触发告警',
      thresholdLabel: '最大更新间隔 (秒)',
    },
    airnodeOffline: {
      label: 'Airnode 离线',
      description: '当 Airnode 离线时自动触发告警',
      thresholdLabel: '离线检测延迟 (秒)',
    },
  },
};
