...
export function UserStatsCard({ stats, loading }: UserStatsCardProps) {
  ...
  if (loading || !stats) {
    return (
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="glass-card p-4 rounded-2xl flex items-center gap-4 animate-pulse">
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {items.map((item, i) => (
        <div key={i} className="glass-card p-4 rounded-2xl flex items-center gap-4 transition-all hover:scale-105">
