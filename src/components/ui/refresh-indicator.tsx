...
import { formatLastUpdated, type RefreshStrategyConfig } from '@/config/refresh-strategy';
 ...
 * 固定在页面右上角
 */
interface PageRefreshIndicatorProps extends RefreshIndicatorProps {
  /** 页面标题 */
  title?: string;
}

export function PageRefreshIndicator({ title, className, ...props }: PageRefreshIndicatorProps) {
  return (
    <div className={cn('flex items-center justify-between border-b bg-white px-4 py-3', className)}>
...
export function TabRefreshIndicator({
  ...
  return (
    <span className="ml-1.5 inline-flex items-center gap-1 text-[10px] text-gray-400">
      {isRefreshing ? (
