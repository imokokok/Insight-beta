...
type TabKey = "activity" | "tvs" | "sync" | "markets" | "accuracy";

type ChartDataItem = ChartItem & {
  cumulativeVolume: number;
};

type SyncChartItem = Omit<SyncMetricItem, "lagBlocks"> & {
  lagBlocks: number | null;
  label: string;
};

type AccuracyChartItem = PricePoint & {
  label: string;
  deviationPct: string;
};
...
type AccuracyAnomaly = PricePoint & {
...
export function OracleCharts({ instanceId }: { instanceId?: string | null }) {
  ...
  const accuracyAnomalies = useMemo<AccuracyAnomaly[]>(() => {
    ...
    return accuracyData
      ...
      .map((p) => {
        const severity =
          p.deviation >= ANOMALY_CRITICAL_THRESHOLD
            ? ("critical" as const)
            : ("warning" as const);
        return {
          ...p,
  ...
  return (
    ...
    >
      ...
      <ChartsHeader activeTab={activeTab} onTabChange={setActiveTab} t={t} />
      <ChartsContent
        activeTab={activeTab}
        t={t}
        chartData={chartData}
