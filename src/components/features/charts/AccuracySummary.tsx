...
function AccuracySummary({
  ...
  if (activeTab !== 'accuracy' || accuracyLoading || !hasAccuracyData) return null;

  return (
    <div className="mt-4 grid gap-4 lg:grid-cols-3">
      <div className="rounded-2xl bg-white/60 p-4 ring-1 ring-black/5 lg:col-span-2">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 px-3 py-2">
            ...
            <div className="text-xl font-black text-emerald-900">{healthScore}</div>
          </div>
      ...
      <div className="rounded-2xl bg-white/60 p-4 ring-1 ring-black/5">
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm font-bold text-gray-900">{t('oracle.charts.anomalyView')}</div>
          <div className="text-[11px] font-semibold text-gray-500">
            {t('oracle.charts.anomalyThreshold')}: {(ANOMALY_WARNING_THRESHOLD * 100).toFixed(1)}%
          </div>
        </div>
        {accuracyAnomalies.length === 0 ? (
          <div className="mt-3 text-xs text-emerald-700">{t('oracle.charts.anomalyNone')}</div>
        ) : (
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-gray-500">
                    {t('oracle.charts.deviationPercent')}: {formatPercent(item.deviation)}
                  </div>
                </div>
                <div
                  className={cn(
