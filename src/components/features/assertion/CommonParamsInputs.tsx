...
export function CommonParamsInputs({
  rule,
  onPatchRuleParams,
  t,
}: CommonParamsInputsProps) {
  return (
    <>
      <div className="md:col-span-6">
        ...
        <Input
          ...
          value={Math.round(
            Number(
              (rule.params as { cooldownMs?: unknown })?.cooldownMs ??
                5 * 60_000,
            ) / 60_000,
          )}
          onChange={(e) => {
      ...
      <div className="md:col-span-6">
        ...
        <Input
          ...
          value={Math.round(
            Number(
              (rule.params as { escalateAfterMs?: unknown })?.escalateAfterMs ??
                0,
            ) / 60_000,
          )}
          onChange={(e) => {
            const minutes = Math.max(
              0,
              Math.min(43200, Math.floor(Number(e.target.value))),
            );
