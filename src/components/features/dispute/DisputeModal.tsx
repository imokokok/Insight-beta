...
export function DisputeModal({
  ...
  const isBondValid =
    Number.isFinite(bondValue) && Number.isFinite(minBondValue) && bondValue >= minBondValue;
  ...
  const hasPublicAddress = (publicEnv.INSIGHT_ORACLE_ADDRESS ?? '').trim().length > 0;
  const shouldWarnMissingConfig = !hasContractAddress && !hasInstanceId && !hasPublicAddress;
    ...
    >
      ...
      >
        <div className="mb-6 flex items-center justify-between">
          <h2 id="dispute-modal-title" className="text-xl font-bold text-gray-900">
        ...
        {shouldWarnMissingConfig && (
          <div className="mb-4 rounded-xl border border-amber-100 bg-amber-50 p-4 text-amber-900">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="rounded-xl border border-rose-100 bg-gradient-to-r from-rose-50 to-amber-50 p-4 text-rose-900">
            <h3 className="flex items-center gap-2 font-semibold">
          ...
          <div>
            ...
            <p className="mb-2 text-xs text-gray-500">{t('oracle.disputeModal.reasonHint')}</p>
            <textarea
              required
              value={reason}
              onChange={(e) => {
