...
describe('DisputeModal', () => {
  ...
  it('renders when open', () => {
    render(<DisputeModal assertionId="0xassertion123" isOpen={true} onClose={mockOnClose} />);
  ...
  it('does not render when closed', () => {
    const { container } = render(
      <DisputeModal assertionId="0xassertion123" isOpen={false} onClose={mockOnClose} />,
    );
  ...
  it('enables submit when config is missing but form is valid', () => {
    ...
    expect(screen.getByRole('button', { name: 'oracle.disputeModal.submit' })).toBeEnabled();
  });
  ...
  it('renders reason helper and example text', () => {
    ...
    expect(screen.getByText('oracle.disputeModal.reasonHint')).toBeInTheDocument();
    expect(
  ...
  it('disables submit when reason is empty', () => {
    ...
    expect(screen.getByRole('button', { name: 'oracle.disputeModal.submit' })).toBeDisabled();
  });
});
