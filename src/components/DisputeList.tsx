...
function getStatusColor(status: DisputeStatus) {
  switch (status) {
    case "Voting":
      return "bg-amber-50 text-amber-700 ring-amber-500/30 ring-1";
    case "Pending Execution":
      return "bg-blue-50 text-blue-700 ring-blue-500/30 ring-1";
    case "Executed":
      return "bg-purple-50 text-purple-700 ring-purple-500/30 ring-1";
    default:
      return "bg-gray-50 text-gray-700 ring-gray-500/30 ring-1";
  }
}

export function DisputeList({ items, loading, viewMode, hasMore, loadMore, loadingMore }: DisputeListProps) {
  ...
  return (
    <div className="space-y-8">
      <div className={cn(
        ...
        {items.map((item) => (
          <Link href={`/oracle/${item.assertionId}`} key={item.id} className="block h-full">
          <div className={cn(
