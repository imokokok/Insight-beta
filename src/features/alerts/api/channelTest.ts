...
async function sendSlackNotification(webhookUrl: string, text: string): Promise<TestResult> {
  if (!webhookUrl) {
    return {
      success: false,
...
async function simulateEmailTest(email: string): Promise<TestResult> {
...
export async function testNotificationChannel(id: string): Promise<TestResult> {
...
If you received this message, your notification channel is configured correctly.`;
  ...
  await updateChannelTestStatus(id, testResult.success ? 'success' : 'failed', testResult.message);
