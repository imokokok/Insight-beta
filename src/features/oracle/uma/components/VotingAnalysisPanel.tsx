    ...
    if (active && payload && payload.length > 0) {
      ...
      return (
        <div className="rounded-lg border border-border bg-card p-3 text-sm shadow-lg">
          ...
          <p className="text-muted-foreground">数量：{item.value}</p>
        </div>
      );
    }
    return null;
  ...
  if (disputes.length === 0) {
    return (
      <div className="rounded-lg border border-border bg-card p-6">
        <h3 className="mb-2 text-lg font-semibold">投票分析</h3>
        <p className="mb-4 text-sm text-muted-foreground">争议投票详情统计</p>
        ...
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-lg border border-border bg-muted/30 p-4 text-center">
              ...
              <div className="text-xs text-muted-foreground">总投票数</div>
            </div>
            <div className="rounded-lg border border-green-500/20 bg-green-500/10 p-4 text-center">
              ...
              <div className="text-xs text-muted-foreground">支持比例</div>
            </div>
            <div className="rounded-lg border border-red-500/20 bg-red-500/10 p-4 text-center">
              <div className="text-2xl font-bold text-red-500">
                {votingStats.opposePercentage.toFixed(1)}%
              </div>
              <div className="text-xs text-muted-foreground">反对比例</div>
            </div>
            <div className="rounded-lg border border-border bg-muted/30 p-4 text-center">
              <div className="text-2xl font-bold">
                {votingStats.totalVotingPower.toLocaleString()}
              </div>
              <div className="text-xs text-muted-foreground">总投票权重</div>
            </div>
          </div>
