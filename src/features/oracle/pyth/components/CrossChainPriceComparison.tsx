...
const PYTH_SUPPORTED_CHAINS: SupportedChain[] = [
  'ethereum',
  'polygon',
  'arbitrum',
  'optimism',
...
export function CrossChainPriceComparison({ isLoading = false }: CrossChainPriceComparisonProps) {
  ...
  const supportedChains = PYTH_SUPPORTED_CHAINS;

  const currentData = useMemo(() => {
    return (
      mockCrossChainData.find((d) => d.symbol === selectedSymbol) ??
      mockCrossChainData[0] ?? {
        symbol: 'BTC',
        prices: [],
        avgPrice: 0,
        maxPrice: 0,
        ...
        <CardHeader>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <CardDescription>对比同一价格源在不同链上的价格差异与一致性</CardDescription>
            </div>
              ...
              <CardContent className="p-4">
                ...
                <div className="mt-2">
                  <span className="text-2xl font-bold">
                    $
                    {currentData.avgPrice.toLocaleString(undefined, {
            ...
            <Card className="bg-muted/50">
              <CardContent className="p-4">
                ...
                <div className="mt-2 flex items-center gap-2">
                  <span className="text-2xl font-bold">
                    $
                    {currentData.priceRange.toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </span>
                  <Badge
                    variant="outline"
                    className={cn(getDeviationColor(currentData.priceRangePercent))}
                  >
            ...
            <Card className="bg-muted/50">
              <CardContent className="p-4">
                ...
                <div className="mt-2 flex items-center gap-2">
                  <span className="text-2xl font-bold">
                    $
                    {currentData.maxDeviation.toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </span>
                  ...
                  return (
                    <TableRow key={chainPrice.chain}>
                      ...
                      <TableCell className="text-right font-semibold">
                        $
                        {chainPrice.price.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          >
                            $
                            {Math.abs(diffFromAvg).toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </span>
                      ...
                      <TableCell className="text-right">
                        <Badge
                          variant="outline"
                          className={cn(getDeviationColor(Math.abs(diffPercent)), 'justify-end')}
                        >
                          {isPositive ? '+' : ''}
