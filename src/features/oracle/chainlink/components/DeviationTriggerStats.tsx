...
interface DeviationTriggerStatsProps {
  collapsible?: boolean;
  className?: string;
}
...
export function DeviationTriggerStats({
  ...
  const renderTop5Feeds = () => {
    if (top5Feeds.length === 0) return null;
      ...
      <div className="rounded-lg border border-border/30 bg-muted/20 p-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          {top5Feeds.map((feed, index) => (
            <div
              key={`${feed.feedName}-${feed.chain}-${index}`}
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>{t('chainlink.deviation.avgUpdateInterval') || '平均更新间隔'}</TableHead>
              <TableHead>{t('chainlink.deviation.lastTriggeredAt') || '最后触发时间'}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
  ...
  if (stats.triggers.length === 0) {
    return (
      <div className={className}>
        <div className="mb-4 flex items-center justify-end">{renderTimeRangeSelector()}</div>
        <div className="flex flex-col items-center justify-center gap-4 py-12">
