...
export interface FetchChainStatusOptions {
  chain?: string;
}

export async function fetchChainStatus(
  options: FetchChainStatusOptions = {},
): Promise<ChainStatusResponse> {
  const validatedOptions = ChainStatusQuerySchema.parse(options);
  return generateChainStatusData(validatedOptions.chain);
}

export { ChainStatusQuerySchema };
