...
type SortBy = 'volume' | 'volatility' | 'updateFrequency' | 'popularity';

function sortPairs(pairs: TrendingPair[], sortBy: SortBy): TrendingPair[] {
  const sorted = [...pairs];
  switch (sortBy) {
    case 'volume':
...
export async function fetchTrending(sortBy: SortBy = 'volume'): Promise<TrendingResponse> {
  const validSortOptions: SortBy[] = ['volume', 'volatility', 'updateFrequency', 'popularity'];
