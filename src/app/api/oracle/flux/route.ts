...
import { startFluxSync, stopFluxSync } from '@/server/oracle/sync/FluxSync';
import {
...
const RATE_LIMITS = {
  GET: { key: 'flux_get', limit: 60, windowMs: 60_000 },
  POST: { key: 'flux_post', limit: 10, windowMs: 60_000 },
  PUT: { key: 'flux_put', limit: 10, windowMs: 60_000 },
  DELETE: { key: 'flux_delete', limit: 5, windowMs: 60_000 },
} as const;
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.error('Flux API GET failed', {
      requestId,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const durationMs = Date.now() - startTime;
    logger.error('Flux API POST failed', {
      requestId,
      error: error instanceof Error ? error.message : 'Unknown',
      durationMs,
    });
...
export async function PUT(request: NextRequest) {
  ...
  try {
    return await handleApi(request, async () => {
      ...
      logger.info('Flux instance updated', { id, updates: Object.keys(updates) });
    ...
    const durationMs = Date.now() - startTime;
    logger.error('Flux API PUT failed', {
      requestId,
      error: error instanceof Error ? error.message : 'Unknown',
      durationMs,
    });
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    return await handleApi(request, async () => {
      ...
      logger.info('Flux instance deleted', { id });
    ...
    const durationMs = Date.now() - startTime;
    logger.error('Flux API DELETE failed', {
      requestId,
      error: error instanceof Error ? error.message : 'Unknown',
      durationMs,
    });
...
async function getOverview() {
  const instances = await listUnifiedInstances({ protocol: 'flux' });
...
async function createInstance(body: Record<string, unknown>) {
  ...
  try {
    ...
    logger.info('Flux instance created', {
      id: instance.id,
      name: instance.name,
      chain: instance.chain,
    });
    ...
    logger.error('Failed to create Flux instance', {
...
async function startSync(instanceId: string) {
  ...
  try {
    ...
    logger.info('Flux sync started', { instanceId });
    ...
    logger.error('Failed to start Flux sync', {
      instanceId,
      error: error instanceof Error ? error.message : String(error),
    });
    return {
...
async function stopSync(instanceId: string) {
  ...
  try {
    ...
    logger.info('Flux sync stopped', { instanceId });
    ...
    logger.error('Failed to stop Flux sync', {
      instanceId,
      error: error instanceof Error ? error.message : String(error),
    });
    return {
