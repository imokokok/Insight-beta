...
describe('GET /api/oracle/assertions/[id]/timeline', () => {
  ...
  it('returns timeline for existing assertion', async () => {
    ...
    const { getAssertion, getDisputeByAssertionId } = await import('@/server/oracle');
    ...
    const request = new Request('http://localhost:3000/api/oracle/assertions/0xassertion/timeline');
  ...
  it('returns not_found when assertion is missing', async () => {
    ...
    type ApiMockResponse = { ok: true; data: unknown } | { ok: false; error: unknown };

    const request = new Request('http://localhost:3000/api/oracle/assertions/0xmissing/timeline');
