...
function getOracleSloIssues(instances: OracleHealthInstance[]) {
  ...
  const maxSyncStalenessMinutes = readSloNumber(
  ...
  for (const inst of instances) {
    const lagNum =
      typeof inst.lagBlocks === "string" ? Number(inst.lagBlocks) : null;
    ...
    if (
      staleMin !== null &&
      Number.isFinite(staleMin) &&
      staleMin > maxSyncStalenessMinutes
    )
...
export async function GET(request: Request) {
  return handleApi(request, async () => {
    ...
    const demoModeEnabled = ["1", "true"].includes(
      env.INSIGHT_DEMO_MODE.toLowerCase(),
    );
    if (probe === "readiness") {
      ...
      const sloIssues = isProd
        ? getOracleSloIssues(oracleHealth.instances)
        : [];
      const oracleReady =
        !isProd || (oracleHealth.issues.length === 0 && sloIssues.length === 0);
      const ready =
        (databaseStatus === "connected" ||
          (!isProd && databaseStatus === "not_configured")) &&
        workerOk &&
        databaseReady &&
        demoReady &&
        oracleReady;
    ...
    if (probe === "validation") {
      ...
      const webhookEnabled = enabledAlertRules.some((rule) =>
        rule.channels?.includes("webhook"),
      );
      const emailEnabled = enabledAlertRules.some((rule) =>
        rule.channels?.includes("email"),
      );
      const telegramEnabled = enabledAlertRules.some((rule) =>
      ...
      if (isProd && databaseStatus === "not_configured")
