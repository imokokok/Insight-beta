...
const OracleCharts = dynamic(
  () => import('@/components/features/oracle/OracleCharts').then((mod) => mod.OracleCharts),
  {
...
const Leaderboard = dynamic(() =>
  import('@/components/features/oracle/Leaderboard').then((mod) => mod.Leaderboard),
);

const PnLCalculator = dynamic(() =>
  import('@/components/features/common/PnLCalculator').then((mod) => mod.PnLCalculator),
);
...
function OracleHealthPanel({
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-8 delay-75 duration-700">
      <div className="glass-panel overflow-hidden rounded-3xl border border-white/60 shadow-2xl shadow-purple-500/5">
        <div className="relative p-6 md:p-8">
          <div className="pointer-events-none absolute inset-0 opacity-40">
            <div
              className={cn(
                'absolute -right-[15%] -top-[40%] h-[200%] w-[50%] rounded-full bg-gradient-to-br blur-3xl',
                health.accent,
              )}
            />
            <div className="absolute bottom-0 left-0 h-[80%] w-[30%] rounded-full bg-gradient-to-tl from-indigo-200/30 via-purple-100/20 to-transparent blur-3xl" />
          </div>
          ...
          <div className="relative z-10 grid gap-6 lg:grid-cols-12">
            <div className="flex items-center gap-6 lg:col-span-5">
              <div
                className={cn(
                  'relative flex h-24 w-24 items-center justify-center rounded-2xl bg-white/70 shadow-sm ring-1 md:h-28 md:w-28',
                  health.ring,
                )}
              >
                <div className="text-center">
                  <div className="text-3xl font-black tabular-nums text-gray-900 md:text-4xl">
                    {health.score ?? '—'}
                  </div>
              ...
              <div className="min-w-0">
                ...
                <div className="text-2xl font-black leading-tight text-gray-900 md:text-3xl">
                  {health.labelText}
                </div>
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-600">
                  ...
                  <div className="rounded-lg bg-white/60 px-3 py-1.5 ring-1 ring-black/5">
                    ...
                    <span className="font-mono font-semibold">
                      {typeof health.activeDisputes === 'number' ? health.activeDisputes : '—'}
                    </span>
                  </div>
                  <div className="rounded-lg bg-white/60 px-3 py-1.5 ring-1 ring-black/5">
                    ...
                    <span className="font-mono font-semibold">
                      {lastSuccessAtText}
                      {lastSuccessAgoText ? (
                        <span className="ml-1 text-gray-400">({lastSuccessAgoText})</span>
                      ) : null}
                    </span>
                  </div>
                </div>
            ...
            <div className="lg:col-span-7">
              <div className="rounded-2xl bg-white/60 p-5 ring-1 ring-black/5 md:p-6">
                ...
                <div className="mt-4 grid gap-2">
                  {health.topRisks.length === 0 ? (
                    <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 px-4 py-3 text-sm text-emerald-700">
                      {health.isLoading ? t('common.loading') : t('oracle.sync.synced')}
                    </div>
                  ) : (
                    health.topRisks.map((r, idx) => (
                      ...
                      >
                        <div className="min-w-0 truncate text-sm font-semibold">{r.key}</div>
                        <div
                          className={cn(
                            'shrink-0 rounded-lg px-2 py-1 text-[11px] font-black uppercase tracking-wider',
                            r.severity === 'critical'
...
function OracleTabs({ activeTab, onChange, t }: OracleTabsProps) {
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 mb-8 flex justify-center delay-200 duration-700">
      <div className="glass-panel flex items-center gap-1 rounded-2xl border-white/60 bg-white/40 p-1.5 shadow-lg shadow-purple-500/5 backdrop-blur-xl">
        <button
          onClick={() => onChange('overview')}
          className={cn(
            'flex items-center gap-2 rounded-xl px-6 py-2.5 text-sm font-bold transition-all duration-300',
            activeTab === 'overview'
              ? 'scale-[1.02] bg-white text-purple-700 shadow-md shadow-purple-500/10 ring-1 ring-black/5'
        ...
        >
          <LayoutDashboard
            ...
            className={cn(
              ...
              activeTab === 'overview' ? 'scale-110 text-purple-600' : 'text-gray-400',
            )}
          />
        ...
        >
          <Trophy
            ...
            className={cn(
              'transition-transform duration-300',
              activeTab === 'leaderboard' ? 'scale-110 text-amber-500' : 'text-gray-400',
            )}
          />
        ...
        >
          <Wrench
            ...
            className={cn(
              'transition-transform duration-300',
              activeTab === 'tools' ? 'scale-110 text-slate-600' : 'text-gray-400',
            )}
          />
          {t('oracle.tabs.tools')}
        </button>
...
function OracleFiltersToolbar({
  ...
  const showClear =
    filterStatus !== 'All' || filterChain !== 'All' || query.trim() || myActivity || myDisputes;

  return (
    <div className="glass-panel sticky top-4 z-20 flex flex-col gap-4 rounded-2xl border-white/60 p-3 shadow-xl shadow-purple-900/5 backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="no-scrollbar flex items-center gap-1.5 overflow-x-auto p-1 sm:p-0">
          {['All', 'Pending', 'Disputed', 'Resolved'].map((status) => (
            <button
              key={status}
              onClick={() => onFilterStatusChange(status as OracleStatus | 'All')}
              className={cn(
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          {address && (
            <div className="flex w-full gap-2 md:hidden">
              <button
                ...
                className={cn(
                  'flex flex-1 items-center justify-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold transition-all',
                  myActivity
                    ? 'border-purple-200 bg-purple-100 text-purple-700 shadow-inner'
                    : 'border-transparent bg-white/50 text-gray-500 hover:bg-white hover:text-gray-900',
                )}
              >
                <User size={14} />
                {t('oracle.myActivity')}
              ...
              <button
                ...
                className={cn(
                  'flex flex-1 items-center justify-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold transition-all',
                  myDisputes
                    ? 'border-rose-200 bg-rose-100 text-rose-700 shadow-inner'
                    : 'border-transparent bg-white/50 text-gray-500 hover:bg-white hover:text-gray-900',
                )}
              >
                <Gavel size={14} />
          ...
          <div className="flex rounded-xl bg-gray-100/50 p-1 md:hidden">
            <button
          ...
          <div className="hidden rounded-xl bg-gray-100/50 p-1 md:flex">
            <button
              onClick={() => onViewModeChange('grid')}
              className={cn(
                'rounded-md p-1.5 transition-all',
          ...
          {instances && instances.length > 0 ? (
            <div className="relative w-full md:hidden">
              <select
                ...
                className="glass-input h-9 w-full cursor-pointer appearance-none rounded-xl border-none pl-3 pr-8 text-sm font-medium text-gray-600 hover:bg-white/80 focus:ring-2 focus:ring-purple-500/20"
              >
                {instances.map((inst) => (
          ...
          <div className="relative w-full md:hidden">
            <select
              value={filterChain}
              onChange={(e) => onFilterChainChange(e.target.value as OracleConfig['chain'] | 'All')}
              className="glass-input h-9 w-full cursor-pointer appearance-none rounded-xl border-none pl-3 pr-8 text-sm font-medium text-gray-600 hover:bg-white/80 focus:ring-2 focus:ring-purple-500/20"
            >
            ...
            <>
              <div className="relative hidden md:block">
                <select
                  value={instanceId}
                  onChange={(e) => onInstanceIdChange(e.target.value)}
                  className="glass-input h-9 cursor-pointer appearance-none rounded-xl border-none pl-3 pr-8 text-sm font-medium text-gray-600 hover:bg-white/80 focus:ring-2 focus:ring-purple-500/20"
                >
                  {instances.map((inst) => (
                    <option key={inst.id} value={inst.id}>
                      {inst.name}
...
export default function OraclePage() {
  ...
  const { items, stats, loading, loadingMore, error, loadMore, hasMore, refresh } = useOracleData(
    filterStatus,
    filterChain,
    debouncedQuery,
  ...
  } = useDisputes('All', filterChain, debouncedQuery, myDisputes ? address : undefined, instanceId);
  ...
  const syncNow = async () => {
    ...
    try {
      ...
      const syncUrl = normalizedInstanceId
        ...
        : '/api/oracle/sync';
      await fetchApiData<{ updated: boolean } | { updated: boolean; chain: unknown }>(syncUrl, {
        method: 'POST',
        headers,
      });
      refresh();
  ...
  const health = useMemo(() => {
    ...
    const lastSuccessMinutes = minutesSince(snapshot?.sync?.lastSuccessAt ?? null);
    ...
    if (typeof consecutiveFailures === 'number' && consecutiveFailures >= 3) score -= 20;
    ...
    if (!hasContract) risks.push({ key: t('errors.missingConfig'), severity: 'critical' });
    if (lastError) risks.push({ key: t('errors.syncFailed'), severity: 'critical' });
  ...
  const lastSuccessAtText = lastSuccessAt ? formatTime(lastSuccessAt, locale) : '—';
  const lastSuccessAgoText =
    typeof health.lastSuccessMinutes === 'number' ? `${health.lastSuccessMinutes}m` : null;
  const lastProcessedBlock = parseOptionalNumber(status?.state?.lastProcessedBlock);
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 space-y-8 pb-20 duration-700">
      <PageHeader title={t('oracle.title')} description={t('oracle.description')}>
        <div className="flex flex-col items-center gap-3 sm:flex-row">
          <ConnectWallet />
          <button
            type="button"
            onClick={() => setIsCreateModalOpen(true)}
            className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-purple-500/25 transition-all hover:shadow-purple-500/40"
          >
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex justify-end">
              <button
                ...
                className="flex items-center gap-2 text-sm font-medium text-gray-500 transition-colors hover:text-purple-600"
              >
                <Settings2 size={16} />
            ...
            {/* Config Panel */}
            {showConfig && (
              <div className="glass-panel animate-in slide-in-from-top-4 fade-in rounded-2xl p-6 duration-300">
                ...
                <div className="mt-6 flex flex-col gap-4 border-t border-gray-100 pt-6 md:flex-row md:items-center md:justify-between">
                  <div className="flex flex-wrap items-center gap-3 text-xs">
                    ...
                    <div className="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-1.5 text-gray-600">
                      <span
                        className={cn(
                          ...
                          status?.state.lagBlocks && Number(status.state.lagBlocks) > 0
                            ? 'bg-amber-500'
                            : 'bg-emerald-500',
                        )}
                      ></span>
                    ...
                    <div className="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-1.5 text-gray-600">
                      ...
                      <span className="font-mono font-medium">
                        {status?.state.rpcActiveUrl ? shortAddress(status.state.rpcActiveUrl) : '—'}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-1.5 text-gray-600">
                      <span className="h-1.5 w-1.5 rounded-full bg-gray-300"></span>
                      ...
                      <span className="font-mono font-medium">
                        {status?.state.owner ? shortAddress(status.state.owner) : '—'}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-1.5 text-gray-600">
                      <span
                    ...
                    <div className="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-1.5 text-gray-600">
                      <span
                        className={cn(
                          'h-1.5 w-1.5 rounded-full',
                          status?.state.syncing ? 'animate-pulse bg-blue-500' : 'bg-emerald-500',
                        )}
                      ></span>
                  ...
                  <button
                    ...
                    disabled={saving}
                    className={cn(
                      'rounded-xl px-6 py-2 text-sm font-medium shadow-sm transition-all',
                      saving
                        ? 'bg-gray-100 text-gray-400'
            ...
            {myDisputes ? (
              ...
              <AssertionList
                ...
                onCreateAssertion={myActivity ? () => setIsCreateModalOpen(true) : undefined}
                instanceId={instanceId}
              />
            )}
          </div>
