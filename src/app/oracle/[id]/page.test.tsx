...
const DisputeModalMock = (props: Record<string, unknown>) => {
  return <div data-testid="dispute-modal">{props.isOpen === true ? 'Dispute Modal' : null}</div>;
};

const SettleModalMock = (props: Record<string, unknown>) => {
  return <div data-testid="settle-modal">{props.isOpen === true ? 'Settle Modal' : null}</div>;
};
...
vi.mock('@/components/features/oracle/PayoutSimulator', () => ({
  PayoutSimulator: () => <div data-testid="payout-simulator">PayoutSimulator</div>,
}));
