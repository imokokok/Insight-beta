...
type RootCauseOption = { value: string; label: string };
...
type OpsSeriesPoint = OpsMetricsSeriesPoint & { label: string };
      ...
      <CardContent className="pt-0">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
          ...
          {alert.entityType &&
            alert.entityId &&
            (() => {
              const rawHref = getEntityHref(alert.entityType, alert.entityId);
              const href = rawHref ? attachInstanceId(rawHref) : null;
              const text = `${alert.entityType}:${alert.entityId.slice(0, 18)}…`;
              return href ? (
                href.startsWith("/api/") ? (
                  <a
                    href={href}
          ...
          {rule?.runbook
            ? (() => {
                const internal = getSafeInternalPath(rule.runbook);
                if (internal) {
                ...
                const external = getSafeExternalUrl(rule.runbook);
                return external ? (
                  <a
                    href={external}
                    target="_blank"
  ...
  return (
    <div className="rounded-xl border border-purple-100/60 bg-white/60 p-3">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-500">
        ...
        {incident.runbook
          ? (() => {
              const internal = getSafeInternalPath(incident.runbook);
              if (internal) {
                if (internal.startsWith("/api/")) {
                  return (
                    <a
              ...
              const external = getSafeExternalUrl(incident.runbook);
              return external ? (
                <a
                  href={external}
                  target="_blank"
        ...
        {incident.entityType && incident.entityId
          ? (() => {
              ...
              const href = rawHref ? attachInstanceId(rawHref) : null;
              const text = `${incident.entityType}:${incident.entityId.slice(0, 18)}…`;
              return href ? (
                href.startsWith("/api/") ? (
                  <a
                    href={href}
                ...
                <div className="flex items-center gap-2">
                  {alert.entityType && alert.entityId
                    ? (() => {
                        ...
                        const href = rawHref ? attachInstanceId(rawHref) : null;
                        const text = `${alert.entityType}:${alert.entityId.slice(0, 14)}…`;
                        return href ? (
                          href.startsWith("/api/") ? (
                            <a
                              href={href}
                          ...
                          <button
                            type="button"
                            onClick={() => onApplyQuery(alert.entityId ?? "")}
                            className="rounded-md bg-gray-50 px-2 py-0.5 border border-gray-100 font-mono hover:bg-white"
                          >
                            {text}
                          </button>
...
export default function AlertsPage() {
  ...
  const rootCauseOptions = useMemo<RootCauseOption[]>(
    () => [
              ...
              items.map((alert) => {
                ...
                return (
                  <AlertCard
                    ...
                    onCreateIncident={() =>
                      createIncidentFromAlert(alert, rule)
                    }
