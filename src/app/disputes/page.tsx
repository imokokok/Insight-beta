...
export default function DisputesPage() {
  ...
  const umaAssertionUrl = (dispute: Dispute) => {
    const id = dispute.assertionId || (dispute.id.startsWith("D:") ? dispute.id.slice(2) : dispute.id);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        params.set("limit", "30");
        const data = await fetchApiData<{ items: Dispute[]; total: number; nextCursor: number | null }>(
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0">
            {(["All", "Voting", "Pending Execution", "Executed"] as const).map((status) => (
              ...
              >
                {status === "All" ? t("common.all") : statusLabel(status as DisputeStatus)}
              </button>
            ))}
          </div>
        ...
        {!loading && items.map((dispute) => (
          <Card key={dispute.id} className="border-purple-100/60 bg-white/60 shadow-sm hover:shadow-md transition-all">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn(
                        ...
                        dispute.status === "Voting" ? "bg-amber-100 text-amber-700 border-amber-200" :
                        dispute.status === "Pending Execution" ? "bg-purple-100 text-purple-700 border-purple-200" :
              ...
              <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-xs mb-1.5">
                        ...
                        <span className="text-emerald-700 font-bold">{calculatePercentage(dispute.currentVotesFor, dispute.totalVotes)}%</span>
                      </div>
                      <div className="h-2 w-full rounded-full bg-emerald-100 overflow-hidden">
                        <div 
                          className="h-full rounded-full bg-emerald-500 transition-all duration-500" 
                          style={{ width: `${calculatePercentage(dispute.currentVotesFor, dispute.totalVotes)}%` }} 
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1.5">
                        ...
                        <span className="text-rose-700 font-bold">{calculatePercentage(dispute.currentVotesAgainst, dispute.totalVotes)}%</span>
                      </div>
                      <div className="h-2 w-full rounded-full bg-rose-100 overflow-hidden">
                        <div 
                          className="h-full rounded-full bg-rose-500 transition-all duration-500" 
                          style={{ width: `${calculatePercentage(dispute.currentVotesAgainst, dispute.totalVotes)}%` }} 
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-xs text-center text-purple-400 pt-2">
                    {t("disputes.totalVotesCast")}: {new Intl.NumberFormat(locale).format(dispute.currentVotesFor + dispute.currentVotesAgainst)}
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <div className="flex justify-center pt-2">
            <button
              ...
              className={cn(
                ...
                loadingMore ? "bg-white/50 text-purple-400" : "bg-white text-purple-600 hover:bg-purple-50 hover:text-purple-700"
              )}
            >
