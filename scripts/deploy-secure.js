async function main() {
  ...
  // Setup multisig signers (in production, use real addresses)
  // For testing, we'll use the first 3 accounts as signers, requiring 2 signatures
  ...
  const InsightOracleSecure = await hre.ethers.getContractFactory(
    "InsightOracleSecure",
    deployer,
  );
  
  const insightOracle = await InsightOracleSecure.deploy(
    tokenAddress,
    initialSigners,
    requiredSignatures
  );
  ...
  console.log(`InsightOracleSecure deployed to: ${address}`);
  if (explorerBase) {
