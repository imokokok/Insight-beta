...
async function main() {
  ...
  console.log('   Used via t()/tn():', usedKeys.size);
  ...
  console.log('\n🔒 Adding directly accessed keys to used list (app.* and errorPage.*)...');
  const protectedNamespaces = ['app', 'errorPage'];
  for (const key of allKeys) {
    if (protectedNamespaces.some(ns => key.startsWith(ns + '.'))) {
      usedKeys.add(key);
    }
  }
  console.log('   Total used keys now:', usedKeys.size);

  const unused = Array.from(allKeys).filter(k => !usedKeys.has(k));
  console.log('\n📊 Unused keys to clean:', unused.length);
